...
(function() {
    ...
    requestAnimationFrame(() => {
      checkboxElements.forEach(element => {
        if (showAll) {
          ...
          // Check if checkbox is checked (has is-list-active class)
          const isChecked = isCheckboxChecked(element);
          
          if (isChecked) {
            // Always show checked checkboxes regardless of search term
            showElement(element);
          } else {
            // For unchecked checkboxes, apply normal filtering
  ...
  function isCheckboxChecked(checkboxElement) {
    // Check if the label has the 'is-list-active' class which indicates it's checked
    const label = checkboxElement.querySelector('label');
    return label && label.classList.contains('is-list-active');
  }
