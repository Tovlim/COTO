...
function selectDistrictCheckbox(districtName) {
  ...
  rafBatch.schedule('selectDistrict', () => {
...
function selectLocalityCheckbox(localityName) {
  ...
  // Batch checkbox operations
  rafBatch.schedule('selectLocality', () => {
...
// OPTIMIZED: Filter application WITHOUT highlighting
...
// OPTIMIZED: Consolidated filter update with single debounce
...
function setupControls() {
  ...
  // OPTIMIZED: Sidebar controls with delegated events
...
// OPTIMIZED: Event setup with consolidated handlers and delegation
function setupEvents() {
  // Use event delegation for auto-sidebar functionality
  eventManager.delegate('change', '[data-auto-sidebar="true"]', () => {
    if (window.innerWidth > 991) {
      state.setTimer('autoSidebar', () => toggleSidebar('Left', true), 50);
    }
  });
  
  eventManager.delegate('change', '[data-auto-second-left-sidebar="true"]', () => {
    if (window.innerWidth > 991) {
      state.setTimer('autoSecondSidebar', () => toggleSidebar('SecondLeft', true), 50);
    }
  });
  
  // Consolidate filter element handlers with single delegated listener
  eventManager.delegate('change', 'select, [fs-cmsfilter-element="select"], [fs-cmsfilter-element="filters"] input, [fs-cmsfilter-element="filters"] select', () => {
    state.setTimer('filterChange', handleFilterUpdate, 50);
  });
  
  // OPTIMIZED: Consolidated apply-map-filter setup with event delegation
  const setupFilterElements = () => {
    // Handle refresh-on-enter specially
    const refreshInput = $id('refresh-on-enter');
    if (refreshInput) {
      eventManager.add(refreshInput, 'keypress', (e) => {
      ...
      eventManager.add(refreshInput, 'input', () => {
    ...
    // Delegate other filter elements
    eventManager.delegate('click', '[apply-map-filter="true"], #refreshDiv, .filterrefresh, #filter-button', (e) => {
  ...
  // Global event listeners with better management - consolidate similar events
  const finsweetEvents = ['fs-cmsfilter-filtered', 'fs-cmsfilter-pagination-page-changed', 
                         'fs-cmsfilter-change', 'fs-cmsfilter-search', 'fs-cmsfilter-reset'];
  
  finsweetEvents.forEach(event => {
    eventManager.add(document, event, (e) => {
      if (window.isMarkerClick || state.markerInteractionLock) return;
      
      // Only trigger filter update for main filter events
      if (event === 'fs-cmsfilter-filtered' || event === 'fs-cmsfilter-pagination-page-changed') {
        handleFilterUpdate();
      }
      
      // Check filtered elements for all events
      rafBatch.schedule('checkFiltered', checkAndToggleFilteredElements, 50);
    });
  });
  
  // Firefox form handling with event delegation
  if (navigator.userAgent.toLowerCase().includes('firefox')) {
    eventManager.delegate('submit', 'form', (e) => {
      const form = e.target;
...
function setupDropdownListeners() {
  ...
  eventManager.delegate('click', '[districtselect]', (e) => {
    if (window.isMarkerClick) return;
...
// OPTIMIZED: Combined GeoJSON loading with Web Worker
function loadCombinedGeoData() {
  const fallbackProcessing = (data) => {
    ...
    state.setTimer('updateDistrictMarkers', () => {
      ...
      state.setTimer('finalLayerOrder', () => mapLayers.debouncedOptimizeLayerOrder(), 300);
  ...
  fetch('https://cdn.jsdelivr.net/gh/Tovlim/COTO@main/Combined-GEOJSON-0.006.json')
      ...
      if (geoJSONWorker) {
        ...
        geoJSONWorker.onmessage = (e) => {
          if (e.data.type === 'success') {
            ...
            // Worker error, use fallback
            fallbackProcessing(combinedData);
          }
        };
        
        geoJSONWorker.postMessage({ type: 'processGeoJSON', data: combinedData });
      } else {
    ...
    .catch(error => {
      ...
      addNativeDistrictMarkers();
      state.setTimer('errorLayerOrder', () => mapLayers.debouncedOptimizeLayerOrder(), 300);
...
function addDistrictBoundaryToMap(name, districtFeature) {
  ...
  if (!existingFeature) {
    const centroid = districtFeature.properties.centroid || utils.calculateCentroid(districtFeature.geometry.coordinates);
...
function loadDistrictTags() {
  ...
  state.setTimer('districtTagsLayerOrder', () => mapLayers.debouncedOptimizeLayerOrder(), 100);
    ...
    forms.forEach(form => {
      if (!form.dataset.filteredElementListener) {
        eventManager.add(form, 'change', () => {
          rafBatch.schedule('checkFiltered', checkAndToggleFilteredElements, 100);
        });
...
function init() {
  ...
  // Layer optimization
  state.setTimer('initialLayerOrder', () => mapLayers.debouncedOptimizeLayerOrder(), 100);
...
map.on("load", () => {
  try {
    ...
    state.setTimer('finalOptimization', () => mapLayers.debouncedOptimizeLayerOrder(), 3000);
...
function initRealTimeAutocomplete() {
    ...
    rafBatch.schedule('initAutocomplete', () => {
...
});// COMBINED MAPBOX & AUTOCOMPLETE SCRIPT - Production Version 2025 - OPTIMIZED
// Includes: Map functionality, Real-time visibility autocomplete, Finsweet integration
// Performance optimizations: RAF batching, event delegation, caching, Web Workers
  ...
  if (type === 'processGeoJSON') {
    try {
      ...
      districts.forEach(district => {
        const centroid = calculateCentroid(district.geometry.coordinates);
        district.properties.centroid = centroid;
      });
      
      self.postMessage({ type: 'success', districts, areas });
    } catch (error) {
...
if (typeof Worker !== 'undefined') {
  const blob = new Blob([geoJSONWorkerCode], { type: 'application/javascript' });
  const workerUrl = URL.createObjectURL(blob);
  geoJSONWorker = new Worker(workerUrl);
}
...
// RAF Batch Manager for combining multiple setTimeout calls
class RAFBatchManager {
  constructor() {
    this.queue = new Map();
    this.rafId = null;
  }
  
  schedule(id, callback, delay = 0) {
    ...
    if (delay > 0) {
      // Use setTimeout for delays, but batch the callbacks
      const timeoutId = setTimeout(() => {
        this.queue.set(id, { callback, timestamp: performance.now() });
        this.scheduleRAF();
      }, delay);
      
      this.queue.set(id, { callback, timeoutId });
    } else {
  ...
  flush() {
    ...
    this.queue.forEach((item, id) => {
      if (item.timestamp && now >= item.timestamp) {
        toExecute.push({ id, callback: item.callback });
      }
    });
    
    // Clear executed items
    toExecute.forEach(({ id }) => this.queue.delete(id));
    ...
    // Schedule next frame if items remain
...
// Global RAF batch manager
const rafBatch = new RAFBatchManager();

// Visibility Observer for efficient element visibility checking
class VisibilityObserver {
  ...
  observe(element, callback) {
    if (!this.observer) {
      ...
      const isVisible = this.isElementVisibleFallback(element);
      this.visibilityMap.set(element, isVisible);
  ...
  isVisible(element) {
    if (this.visibilityMap.has(element)) {
      return this.visibilityMap.get(element);
    }
    
    // Fallback check
    return this.isElementVisibleFallback(element);
  }
  
  isElementVisibleFallback(element) {
    let current = element;
...
// ========================
// AUTOCOMPLETE CLASS DEFINITION (OPTIMIZED)
// ========================
...
class RealTimeVisibilityAutocomplete {
    ...
    handleFocus() {
        ...
        // Clear visibility cache on focus
    ...
    handleClear() {
        if (this.elements.input.value) {
            ...
            // Clear cache when input is cleared
            this.clearVisibilityCache();
        }
    }
    ...
    setActiveItem(items, index) {
        rafBatch.schedule('setActiveItem', () => {
    ...
    clearVisibilityCache() {
        this.visibilityCache.districts = null;
        this.visibilityCache.localities = null;
        this.visibilityCache.lastUpdate = 0;
    }
    
    // OPTIMIZED: Use cached visibility data
    getCurrentlyVisibleDistricts() {
        const now = Date.now();
        if (this.visibilityCache.districts && 
            (now - this.visibilityCache.lastUpdate) < this.visibilityCache.cacheTimeout) {
            return this.visibilityCache.districts;
        }
        ...
        this.visibilityCache.districts = [...visibleDistricts].sort();
        this.visibilityCache.lastUpdate = now;
    ...
    getCurrentlyVisibleLocalities() {
        const now = Date.now();
        if (this.visibilityCache.localities && 
            (now - this.visibilityCache.lastUpdate) < this.visibilityCache.cacheTimeout) {
            return this.visibilityCache.localities;
        }
        ...
        this.visibilityCache.localities = [...visibleLocalities].sort();
    ...
    selectTerm(term, type = 'locality') {
        ...
        rafBatch.schedule('focusAfterSelect', () => {
    ...
    triggerDistrictSelection(districtName) {
        ...
        if (districtFeature && districtFeature.properties.source === 'tag') {
            ...
            rafBatch.schedule('districtFilterUpdate', () => {
                if (window.mapUtilities && window.mapUtilities.state) {
                    ...
                    if (typeof window.applyFilterToMarkers === 'function') {
                        ...
                        rafBatch.schedule('districtCleanup', cleanupFlags, 1000);
                    } else {
                        // Clean up if no applyFilterToMarkers function
                        rafBatch.schedule('districtCleanup', cleanupFlags, 500);
                    }
                } else {
        ...
        if (window.mapUtilities && window.mapUtilities.state && typeof window.highlightBoundary === 'function') {
            ...
            if (window.map && window.map.getSource && window.map.getSource(boundarySourceId)) {
                ...
                if (source && source._data) {
                    ...
                    rafBatch.schedule('boundaryCleanup', cleanupFlags, 1200);
        ...
        rafBatch.schedule('districtFallback', () => {
            if (window.mapUtilities && window.mapUtilities.state) {
                const state = window.mapUtilities.state;
                state.flags.forceFilteredReframe = true;
                state.flags.isRefreshButtonAction = true;
                ...
                if (typeof window.applyFilterToMarkers === 'function') {
                    ...
                    rafBatch.schedule('fallbackCleanup', cleanupFlags, 1000);
                } else {
                    // Clean up if no applyFilterToMarkers function
                    rafBatch.schedule('fallbackCleanup', cleanupFlags, 500);
                }
            } else {
                // Clean up if no mapUtilities
                rafBatch.schedule('fallbackCleanup', cleanupFlags, 500);
    ...
    triggerLocalitySelection(localityName) {
        ...
        // Fly to locality's coordinates instead of reframing to all filtered
        ...
        if (localityFeature && window.map) {
            ...
            rafBatch.schedule('localityCleanup', cleanupFlags, 1200);
        } else {
            // Fallback: If locality not found, just clean up
            rafBatch.schedule('localityCleanup', cleanupFlags, 500);
        }
    ...
    triggerSearchEvents() {
        ...
        if (window.fsAttributes?.cmsfilter) {
            rafBatch.schedule('finsweetReload', () => {
    ...
    getStats() {
        ...
        return {
            ...
            cacheStatus: this.visibilityCache.districts ? 'cached' : 'empty'
        };
    }
}
...
// Reduced fallback timeout from 20s to 10s
setTimeout(() => {
  const loadingScreen = document.getElementById('loading-map-screen');
...
// Reduced UI state timeout from 12s to 6s
setTimeout(() => {
...
// OPTIMIZED: Comprehensive DOM Element Cache with size limits
class OptimizedDOMCache {
  ...
  $1(selector) {
    if (!this.selectorCache.has(selector)) {
      ...
      this.enforceLimit(this.selectorCache);
    }
  ...
  $(selector) {
    if (!this.listCache.has(selector)) {
      ...
      this.enforceLimit(this.listCache);
    }
...
// OPTIMIZED: Event Listener Management System with delegation
  ...
  setupDelegation() {
    ...
    ['click', 'change', 'input'].forEach(eventType => {
      document.addEventListener(eventType, (e) => {
        const listeners = this.delegatedListeners.get(eventType);
        if (!listeners) return;
...
class OptimizedMapState {
  ...
  setTimer(id, callback, delay) {
    // Use RAF batch manager instead of direct setTimeout
    rafBatch.schedule(id, callback, delay);
  }
...
// ENHANCED: Loading state tracker with priorities
const loadingTracker = {
  ...
  criticalStates: ['mapInitialized', 'locationDataLoaded', 'markersAdded'],
  ...
  checkAllComplete() {
    // Check if critical states are complete
    const criticalComplete = this.criticalStates.every(state => this.states[state]);
  ...
  showPartialUI() {
    // Enable map interaction even if some UI elements aren't ready
  ...
  checkAutocompleteReady() {
    if (window.integratedAutocomplete && !this.states.autocompleteReady) {
      ...
      rafBatch.schedule('checkAutocomplete', () => this.checkAutocompleteReady(), 500);
    }
  }
};
...
const utils = {
  ...
  setStyles: (el, styles) => {
    ...
    rafBatch.schedule(`style-${el.id || Math.random()}`, () => {
...
// OPTIMIZED: Map Layer Management System with debounced reordering
class OptimizedMapLayers {
  ...
  addToBatch(operation) {
    ...
    if (!this.pendingBatch) {
      ...
      rafBatch.schedule('mapLayerBatch', () => this.processBatch());
    }
  }
  ...
  processBatch() {
    this.batchOperations.forEach(operation => {
      try {
        ...
        console.error('Map layer batch error:', error);
      }
    });
    ...
    this.debouncedOptimizeLayerOrder();
  }
  
  // Debounced layer reordering
  debouncedOptimizeLayerOrder() {
      ...
      markerLayers.forEach(layerId => {
        if (this.hasLayer(layerId)) {
          try {
            ...
            console.error('Layer reorder error:', e);
          }
        }
      });
    }
