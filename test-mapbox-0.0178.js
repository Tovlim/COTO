...
// Optimized: Memory management, network handling, smart loading, performance improvements

// ========================
// WEB WORKER FOR GEOJSON PROCESSING
// ========================

// Create inline worker for GeoJSON processing
...
self.onmessage = function(e) {
  ...
  if (type === 'processGeoJSON') {
    ...
    self.postMessage({ type: 'processed', districts, areas });
  }
};
`;
...
if (typeof Worker !== 'undefined') {
  try {
    geoJSONWorker = new Worker(geoJSONWorkerURL);
  } catch (e) {
...
class RealTimeVisibilityAutocomplete {
    constructor(options = {}) {
        ...
        // Visibility cache with expiration
        this.visibilityCache = new Map();
        this.visibilityCacheExpiry = 2000; // 2 seconds
    ...
    applyStyles() {
        ...
        // Minimize CSS by reusing existing styles
        const existingStyle = document.querySelector(`#${this.elementIds.wrapperId}-scrollbar-style`);
        if (!existingStyle) {
    ...
    isElementVisible(element) {
        ...
        if (this.visibilityObserver && this.observedElements.has(element)) {
    ...
    getCurrentlyVisibleTerms() {
        ...
        if (this.visibilityCache.size > 0 && (now - this.lastVisibilityCheck) < this.visibilityCacheExpiry) {
            return Array.from(this.visibilityCache.keys());
        }
        ...
        try {
            ...
            // Start observing elements if using Intersection Observer
            if (this.visibilityObserver) {
                [...localityElements, ...districtElements].forEach(el => {
    ...
    getCurrentlyVisibleDistricts() {
        ...
        if (this.visibilityCache.size > 0 && (Date.now() - this.lastVisibilityCheck) < this.visibilityCacheExpiry) {
            const districts = [];
            this.visibilityCache.forEach((type, term) => {
                if (type === 'district') districts.push(term);
            });
            return districts.sort();
        }
        
        // Fallback to full check
        return this.getCurrentlyVisibleTerms().filter(term => {
            return this.visibilityCache.get(term) === 'district';
        });
    }
    ...
    getCurrentlyVisibleLocalities() {
        ...
        if (this.visibilityCache.size > 0 && (Date.now() - this.lastVisibilityCheck) < this.visibilityCacheExpiry) {
            const localities = [];
            this.visibilityCache.forEach((type, term) => {
                if (type === 'locality') localities.push(term);
            });
        ...
        // Fallback to full check
        return this.getCurrentlyVisibleTerms().filter(term => {
            return this.visibilityCache.get(term) === 'locality';
        });
    }
    ...
    addDistrictStyling() {
        ...
        style.textContent = `.list-term.district-term,.list-term.locality-term{display:flex;align-items:center;justify-content:space-between;padding:8px 12px}.list-term.district-term{font-weight:600;color:#6e3500;background-color:#fdf6f0;border-left:3px solid #6e3500}.list-term.district-term:hover{background-color:#f5e6d3}.list-term.locality-term{font-weight:500;color:#7e7800;background-color:#fffef5;border-left:3px solid #7e7800}.list-term.locality-term:hover{background-color:#f9f8e6}.term-label{font-size:0.75em;font-weight:normal;opacity:0.8;margin-left:8px;flex-shrink:0}.list-term.district-term .term-label{color:#8f4500}.list-term.locality-term .term-label{color:#a49c00}`;
        document.head.appendChild(style);
    }
...
// Smart loading fallback - reduced from 20s to 8s
setTimeout(() => {
  const loadingScreen = document.getElementById('loading-map-screen');
...
// Optimized fallback - mark non-critical UI states as complete after 5s
setTimeout(() => {
  const nonCriticalStates = ['tabSwitcherSetup', 'autocompleteReady', 'backToTopSetup'];
  nonCriticalStates.forEach(state => {
    if (!loadingTracker.states[state]) {
      loadingTracker.markComplete(state);
    }
  });
}, 5000);
...
class OptimizedDOMCache {
  ...
  $1(selector) {
    this._trackAccess(selector);
    if (!this.selectorCache.has(selector)) {
      this._enforceLimit(this.selectorCache);
  ...
  $(selector) {
    ...
    if (!this.listCache.has(selector)) {
      this._enforceLimit(this.listCache);
  ...
  _enforceLimit(cache) {
    if (cache.size >= this.maxCacheSize) {
      ...
      const toRemove = entries
        ...
        .slice(0, Math.floor(this.maxCacheSize * 0.2)); // Remove 20%
...
setInterval(() => domCache.cleanup(), 60000); // Every minute
...
class OptimizedEventManager {
  constructor() {
    ...
    this.debounceTimers = new Map();
    this.globalHandlers = new Map(); // For consolidated global handlers
  }
  ...
  // Enhanced event delegation with global handler consolidation
  delegate(parentSelector, event, childSelector, handler) {
    // Try to use existing global handler for this event type
    let globalHandler = this.globalHandlers.get(event);
    
    if (!globalHandler) {
      // Create new global handler
      globalHandler = (e) => {
        // Check all registered selectors for this event
        const delegates = this.delegatedListeners.get(event) || [];
        delegates.forEach(({ childSelector, handler }) => {
      ...
      document.addEventListener(event, globalHandler, true);
      this.globalHandlers.set(event, globalHandler);
    }
    
    // Register the selector and handler
...
// OPTIMIZED: Smart State Management with RAF batching
...
// ENHANCED: Loading state tracker with priority system
const loadingTracker = {
  ...
  critical: ['mapInitialized', 'locationDataLoaded', 'markersAdded'],
  ...
  checkAllComplete() {
    ...
    const criticalComplete = this.critical.every(state => this.states[state]);
    
    if (criticalComplete) {
      // Show map even if other features still loading
      this.showPartialUI();
    }
  ...
  showPartialUI() {
    const loadingScreen = document.getElementById('loading-map-screen');
    if (loadingScreen) {
      loadingScreen.style.opacity = '0.3'; // Fade loading screen
    }
  },
...
// OPTIMIZED: Debounced filter update without redundant triggers
const debouncedFilterUpdate = eventManager.debounce(() => {
...
}, 300, 'globalFilterUpdate');

// Create single handler function to avoid duplicates
...
function setupEvents() {
  ...
  eventManager.delegate('body', 'change', '[data-auto-sidebar="true"]', () => {
    if (window.innerWidth > 991) {
      state.setTimer('autoSidebar', () => toggleSidebar('Left', true), 50);
    }
  });
  
  eventManager.delegate('body', 'change', '[data-auto-second-left-sidebar="true"]', () => {
    if (window.innerWidth > 991) {
  ...
  eventManager.delegate('body', 'change', 'select, [fs-cmsfilter-element="select"], [fs-cmsfilter-element="filters"] input, [fs-cmsfilter-element="filters"] select', handleFilterUpdate);
  
  // OPTIMIZED: Consolidated apply-map-filter setup
  ...
  if (navigator.userAgent.toLowerCase().includes('firefox')) {
    eventManager.delegate('body', 'submit', 'form', (e) => {
      const form = e.target;
  ...
  eventManager.delegate('body', 'click', 'a:not(.filterrefresh):not([fs-cmsfilter-element])', (e) => {
    const link = e.target;
...
function setupDropdownListeners() {
  ...
  eventManager.delegate('body', 'click', '[districtselect]', (e) => {
    if (window.isMarkerClick) return;
...
// OPTIMIZED: Combined GeoJSON loading with web worker and retry logic
function loadCombinedGeoData() {
  const url = 'https://cdn.jsdelivr.net/gh/Tovlim/COTO@main/Combined-GEOJSON-0.006.json';
    ...
    .then(combinedData => {
      ...
      if (geoJSONWorker) {
        ...
        geoJSONWorker.onmessage = (e) => {
          ...
          processGeoJSONData(districts, areas);
        };
        
        geoJSONWorker.onerror = () => {
          // Fallback to main thread processing
          console.log('Worker error, falling back to main thread');
          processGeoJSONInMainThread(combinedData);
        };
      } else {
        // Process in main thread if no worker
        processGeoJSONInMainThread(combinedData);
      }
    })
    .catch(error => {
      console.error('Failed to load GeoJSON after retries:', error);
...
function processGeoJSONInMainThread(combinedData) {
  const districts = [];
  const areas = [];
...
function processGeoJSONData(districts, areas) {
