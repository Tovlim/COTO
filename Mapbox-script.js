Pasted content
52.45 KB •1,425 lines•Formatting may be inconsistent from source
// Initialize Mapbox
...
const map = new mapboxgl.Map({
  ...
  zoom: 8.33, // Start at West Bank zoom level
...
function addCustomMarkers() {
  ...
  locationData.features.forEach((feature, i) => {
    ...
    const currentZoom = map.getZoom();
    const shouldInitiallyShow = currentZoom >= 9;
    ...
    if (!shouldInitiallyShow || isInitialLoad) {
      ...
      console.log(`🚀 Creating marker for ${name} - initially hidden (zoom: ${currentZoom.toFixed(2)}, isInitialLoad: ${isInitialLoad})`);
    } else {
      el.style.opacity = '1';
      el.style.visibility = 'visible';
      el.style.display = 'block';
      el.style.pointerEvents = 'auto';
      console.log(`🚀 Creating marker for ${name} - initially visible (zoom: ${currentZoom.toFixed(2)})`);
    }
...
function checkOverlap() {
  ...
  console.log(`🔍 Zoom check: ${currentZoom.toFixed(2)} >= 9? ${shouldShowMarkers}`);
  ...
  positions.forEach((info, idx) => {
    if (isInitialLoad && map.getZoom() < 9) {
      console.log(`⏸️ Skipping marker visibility for ${info.name} during initial load (zoom too low)`);
      return;
    }
