...
(function(window) {
  ...
  // ====================================================================
  // INJECT OPTIMIZED CSS FOR SIDEBARS
  // ====================================================================
  (function injectSidebarStyles() {
    if (document.getElementById('sidebar-optimized-styles')) return;
    
    const style = document.createElement('style');
    style.id = 'sidebar-optimized-styles';
    style.textContent = `
      .sidebar-transitions {
      ...
      #LeftSidebar.sidebar-transitions {
        transition-property: margin-left;
      }
      #RightSidebar.sidebar-transitions {
        transition-property: margin-right;
      }
      #SecondLeftSidebar.sidebar-transitions {
        transition-property: margin-secondleft;
      }
    `;
    
    // Insert at the beginning of head for early styling
  ...
  const sidebarCache = {
    ...
    // Pre-cache all sidebar elements at once
    initialize() {
      ...
      ['Left', 'Right', 'SecondLeft'].forEach(side => {
        ...
        if (sidebar) {
          this.elements.set(side, sidebar);
          // Pre-calculate width once
          this.widths.set(side, sidebar.offsetWidth || 300);
          
          // Cache arrow element
          const arrowKey = side.toLowerCase();
          const arrow = document.querySelector(`[arrow-icon="${arrowKey}"]`);
          if (arrow) this.arrows.set(side, arrow);
        }
      });
  ...
  function setupSidebars() {
    ...
    const setupSidebarElement = (side) => {
      ...
      if (sidebar.dataset.fastSetup === 'true') return true;
      
      // Add optimized CSS class instead of inline styles
      sidebar.classList.add('sidebar-transitions');
      sidebar.style.zIndex = zIndex;
      sidebar.style.position = 'relative';
      
      if (tab) {
        tab.classList.add('sidebar-tab-transitions');
      }
      ...
      const bringToFront = () => {
        ...
        if (window.innerWidth <= 478 && tab) {
          tab.style.zIndex = newZ + 10;
          if (tab.parentElement) tab.parentElement.style.zIndex = newZ + 10;
        }
        
        // Optimize z-index updates
        sidebarCache.elements.forEach((otherSidebar, otherSide) => {
          if (otherSide !== side && otherSidebar) {
            otherSidebar.style.zIndex = newZ - 1;
          }
        });
      };
      ...
      if (tab && tab.dataset.setupComplete !== 'true') {
      ...
      if (close && close.dataset.setupComplete !== 'true') {
      ...
      sidebar.dataset.fastSetup = 'true';
      zIndex++;
      return true;
    };
    ...
    const setupInitialMargins = () => {
      ...
      sidebarCache.elements.forEach((sidebar, side) => {
    ...
    // Setup all sidebars immediately without retries
    const setupComplete = ['Left', 'Right', 'SecondLeft'].every(side => setupSidebarElement(side));
    ...
    // Use MutationObserver for truly dynamic sidebars
    if (!setupComplete) {
      const observer = new MutationObserver((mutations) => {
        for (const side of ['Left', 'Right', 'SecondLeft']) {
          const sidebar = document.getElementById(`${side}Sidebar`);
          if (sidebar && sidebar.dataset.fastSetup !== 'true') {
            setupSidebarElement(side);
          }
        }
      });
  ...
  function initializeCore() {
    // Setup sidebars first - this is the priority
    setupSidebars();
  ...
  // Initialize sidebars immediately for fastest load
  setupSidebars();
  ...
  if (document.readyState === 'loading') {
    document.addEventListener('DOMContentLoaded', () => {
      ...
      // Initialize remaining core functions
      IdleExecution.schedule(() => {
        setupEvents();
        setupLocationTabListener();
        checkAndToggleFilteredElements(true);
  ...
  window.addEventListener('load', () => {
    // Re-initialize sidebar cache in case DOM changed
    ...
    if (checkForFieldItems()) {
      IdleExecution.scheduleUI(processFieldItemsDebounced, { fallbackDelay: 400 });
    }
  });
