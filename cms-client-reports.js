...
(function() {
    ...
    const CONFIG = {
        ...
        USE_DATA_WORKER: true // Enable Web Worker for off-main-thread data processing
    };
    ...
    const ExtraVisibility = {
        ...
        apply(itemElement, reportData) {
            ...
            // Use pre-computed visibility from worker if available
            const precomputed = reportData.visibility;
            ...
            extraElements.forEach(element => {
                ...
                const shouldShow = subjects.every(subj => {
                    // Use pre-computed visibility if available
                    if (precomputed && subj in precomputed) {
                        return precomputed[subj];
                    }
        ...
        hasData(subject, reportData) {
            ...
            // Use pre-computed visibility if available
            if (reportData.visibility && subj in reportData.visibility) {
                return reportData.visibility[subj];
            }
            // Fallback to inline check
            const check = this._subjectChecks[subj];
    ...
    // Handles: fetch, JSON parsing, date formatting, URL building, visibility computation
    const DataWorker = {
        _worker: null,
        _requestId: 0,
        ...
        _workerCode: `
            ...
            const DateUtils = {
                ...
                format(dateString, now) {
                    if (!dateString) return '';
                    try {
                        const date = new Date(dateString);
                        if (isNaN(date.getTime())) return '';

                        const daysDiff = this._getDaysDifference(date, now);

                        if (daysDiff === 0) return 'Today';
                        if (daysDiff === 1) return 'Yesterday';
                        if (daysDiff >= 2 && daysDiff <= 13) return daysDiff + ' days ago';
            ...
            function computeVisibility(data) {
                return {
                    perpetrator: !!(data.perpetrators?.length > 0 || data.perpetrator),
                    perp: !!(data.perpetrators?.length > 0 || data.perpetrator),
                    settlement: !!(data.settlement?.name || data.settlement?.slug),
                    place: !!(data.place?.name || data.place?.slug || data.locationType?.name || data.locationType?.slug),
                    locationType: !!(data.place?.name || data.place?.slug || data.locationType?.name || data.locationType?.slug),
                    backer: !!(data.backer?.name || data.backer?.slug),
                    description: !!(data.description?.trim()),
                    info: !!(data.description?.trim()),
                    videos: !!(data.videos?.length > 0),
                    images: !!(data.reportImages?.length > 0),
                    reportImages: !!(data.reportImages?.length > 0),
                    photo: !!(data.photo?.url),
                    victimsDonationLink: !!(data.victimsDonationLink),
                    victims: !!(data.victimsDonationLink),
                    reporterEventLink: !!(data.reporterEventLink),
                    urgent: data.urgent === true,
                    topic: !!(data.topic?.length > 0 || data.topic?.slug),
                    reporters: !!(data.reporters?.length > 0),
                    reporter: !!(data.reporters?.length > 0),
                    locality: !!(data.locality?.name || (Array.isArray(data.locality) && data.locality.length > 0)),
                    region: !!(data.subRegion?.name || data.region?.name ||
                              (Array.isArray(data.subRegion) && data.subRegion.length > 0) ||
                              (Array.isArray(data.region) && data.region.length > 0)),
                    territory: !!(data.territory?.name || (Array.isArray(data.territory) && data.territory.length > 0)),
                    multiLocality: !!data.hasMultipleLocalities,
                    multiRegion: !!data.hasMultipleSubRegions,
                    multiTerritory: !!data.hasMultipleTerritories,
                    multiGovernorate: !!data.hasMultipleRegions
                };
            }

            // ===== TRANSFORM SINGLE REPORT =====
            function transformReport(raw, now) {
                const dateValue = raw.date || raw.createdOn;
                const reportDate = dateValue ? new Date(dateValue) : null;
                const daysDiff = reportDate && !isNaN(reportDate.getTime())
                    ? DateUtils._getDaysDifference(reportDate, now)
                    : Infinity;

                // Pre-compute urgent badge visibility (urgent AND within 13 days)
                const showUrgentBadge = raw.urgent === true && daysDiff <= 13;

                // Pre-build reporter data with URLs
                const reporters = (raw.reporters || []).map(r => ({
                    name: r.name,
                    slug: r.slug,
                    url: r.slug ? '/reporter/' + r.slug : null,
                    photoUrl: r.photo?.url || r.photo || null,
                    donationLink: r.donationLink || null,
                    joinLink: r.joinLink || null
                }));

                // Pre-build topic data with URLs
                const topics = Array.isArray(raw.topic)
                    ...
                    : raw.topic?.slug
                        ? [{ name: raw.topic.name, slug: raw.topic.slug, url: '/topic/' + raw.topic.slug }]
                        : [];

                // Normalize location fields
                const normalizeLocation = (data, urlPrefix) => {
                    if (!data) return null;
                    if (Array.isArray(data) && data.length > 0) {
                        return data.map(item => ({
                            name: item.name,
                            slug: item.slug,
                            url: item.slug ? urlPrefix + item.slug : null
                        }));
                    }
                    if (data.name) {
                        return [{
                            name: data.name,
                            slug: data.slug,
                            url: data.slug ? urlPrefix + data.slug : null
                        }];
                    }
                    return null;
                };

                // Pre-build perpetrator info
                let perpetrator = null;
                if (raw.perpetrators?.length > 0) {
                    perpetrator = {
                        name: raw.perpetrators[0].name,
                        slug: raw.perpetrators[0].slug,
                        url: raw.perpetrators[0].slug ? '/perp/' + raw.perpetrators[0].slug : null
                    };
                } else if (raw.perpetrator) {
                    perpetrator = {
                        name: raw.perpetrator.name || raw.perpetrator,
                        slug: raw.perpetrator.slug,
                        url: raw.perpetrator.slug ? '/perp/' + raw.perpetrator.slug : null
                    };
                }

                // Pre-build settlement info
                const settlement = raw.settlement ? {
                    name: raw.settlement.name || raw.settlement,
                    slug: raw.settlement.slug,
                    url: raw.settlement.slug ? '/settlement/' + raw.settlement.slug : null
                } : null;

                // Pre-build place/locationType info
                const place = (raw.place || raw.locationType) ? {
                    name: raw.place?.name || raw.locationType?.name || raw.place || raw.locationType,
                    slug: raw.place?.slug || raw.locationType?.slug,
                    url: (raw.place?.slug || raw.locationType?.slug)
                        ? '/place/' + (raw.place?.slug || raw.locationType?.slug)
                        : null
                } : null;

                // Pre-build backer info
                const backer = raw.backer ? {
                    name: raw.backer.name || raw.backer,
                    slug: raw.backer.slug,
                    url: raw.backer.slug ? '/backer/' + raw.backer.slug : null
                } : null;

                // Pre-process videos with embed URLs
                ...
                // Build report URL
                const reportUrl = raw.reporterEventLink ||
                    (raw.slug ? 'https://occupationcrimes.org/report/' + raw.slug : null);
                ...
                return {
                    ...
                    name: raw.name,
                    formattedDate: DateUtils.format(dateValue, now),
                    formattedDateForTag: DateUtils.formatForTag(dateValue),
                    reporterNamesFormatted: formatReporterNames(raw.reporters),
                    daysDiff: daysDiff,

                    // Pre-built URLs
                    reportUrl: reportUrl,
                    reporterEventLink: raw.reporterEventLink || null,
                    victimsDonationLink: raw.victimsDonationLink || null,

                    // Normalized arrays with pre-built URLs
                    reporters: reporters,
                    topics: topics,

                    // Location data normalized with URLs
                    localities: normalizeLocation(raw.locality, '/locality/'),
                    subRegions: normalizeLocation(raw.subRegion, '/region/'),
                    regions: normalizeLocation(raw.region, '/region/'),
                    territories: normalizeLocation(raw.territory, '/territory/'),

                    // Multi-location flags
                    hasMultipleLocalities: !!raw.hasMultipleLocalities,
                    hasMultipleSubRegions: !!raw.hasMultipleSubRegions,
                    hasMultipleRegions: !!raw.hasMultipleRegions,
                    hasMultipleTerritories: !!raw.hasMultipleTerritories,

                    // Perpetrator info normalized
                    perpetrator: perpetrator,
                    perpetrators: raw.perpetrators || [],
                    settlement: settlement,
                    place: place,
                    backer: backer,

                    // Media
                    photoUrl: raw.photo?.url || null,
                    photoAlt: raw.name || '',
                    reportImages: raw.reportImages || [],
                    videos: videos,

                    // Pre-computed visibility flags
                    visibility: computeVisibility(raw),

                    // Pre-computed UI state
                    showUrgentBadge: showUrgentBadge,
                    hasMultipleReporters: reporters.length > 1,

                    // Raw content (still needed for rich text)
                    description: raw.description || '',

                    // Urgent flag
                    urgent: raw.urgent || false,

                    // Original raw data for fields we haven't transformed
            ...
            self.onmessage = async function(e) {
                const { type, requestId, url, isAppend, reports } = e.data;

                if (type === 'CANCEL_REQUEST') {
                    // Cancellation is handled by main thread ignoring stale responses
                    self.postMessage({ type: 'REQUEST_CANCELLED', requestId });
                    return;
                }

                if (type === 'TRANSFORM_CACHED') {
                    // Transform cached data without fetch
                    try {
                        const now = new Date();
                        const transformedReports = reports.map(r => transformReport(r, now));
                        self.postMessage({
                            type: 'REPORTS_READY',
                            requestId,
                            data: transformedReports,
                            metadata: { total: transformedReports.length }
                        });
                    } catch (error) {
                        self.postMessage({
                ...
                if (type === 'FETCH_REPORTS') {
                    try {
                        // Perform fetch in worker (off main thread)
                        ...
                        // Parse JSON in worker (can be 10-50ms for large responses)
                        const responseData = await response.json();
                        const items = responseData.data || [];

                        // Transform all reports
                        const now = new Date();
                        const transformedReports = items.map(r => transformReport(r, now));
                        ...
                        self.postMessage({
                            ...
                            requestId,
                            data: transformedReports,
                            metadata: responseData.metadata || { total: transformedReports.length },
                            isAppend: isAppend
                        });
        ...
        init() {
            ...
            this._initPromise = new Promise((resolve, reject) => {
                if (!CONFIG.USE_DATA_WORKER || typeof Worker === 'undefined') {
                    console.log('[CMS Client] DataWorker disabled or unsupported, using main thread');
                    this._initialized = false;
                    resolve(false);
                    return;
                }
                ...
                try {
                    ...
                    this._worker.onmessage = (e) => {
                        ...
                        if (type === 'WORKER_READY') {
                            this._initialized = true;
                            console.log('[CMS Client] DataWorker initialized');
                            resolve(true);
                            return;
                        }
                        ...
                        if (pending) {
                            ...
                            } else if (type === 'FETCH_ERROR' || type === 'TRANSFORM_ERROR') {
                    ...
                    this._worker.onerror = (error) => {
                        console.error('[CMS Client] DataWorker error:', error);
                        this._initialized = false;
                        reject(error);
                    };
                    ...
                    console.error('[CMS Client] Failed to create DataWorker:', error);
                    this._initialized = false;
                    resolve(false);
                }
            });
        ...
        isAvailable() {
            return this._initialized && this._worker !== null;
        },

        // Fetch and transform reports via worker
        fetchReports(url, isAppend = false) {
            if (!this.isAvailable()) {
                return this._fallbackFetch(url, isAppend);
            }
        ...
        // Transform cached reports via worker (for view switching)
        transformCached(reports) {
            if (!this.isAvailable()) {
                return this._fallbackTransform(reports);
            }
        ...
        cancel(requestId) {
            if (this._worker && requestId) {
                this._worker.postMessage({
                    type: 'CANCEL_REQUEST',
                    requestId
                });
            }
            // Invalidate by incrementing request ID
            this._requestId++;
        },

        // Get current request ID (for race condition checking)
        getCurrentRequestId() {
            return this._requestId;
        },

        // Fallback: fetch on main thread when worker unavailable
        async _fallbackFetch(url, isAppend = false) {
            const response = await fetch(url);
            if (!response.ok) {
                throw new Error(`HTTP ${response.status}: ${response.statusText}`);
            }
            ...
            // Transform on main thread
            const transformedReports = this._transformReportsMainThread(items);
        ...
        // Fallback: transform on main thread
        async _fallbackTransform(reports) {
            const transformedReports = this._transformReportsMainThread(reports);
            return {
                type: 'REPORTS_READY',
                data: transformedReports,
                metadata: { total: transformedReports.length }
        ...
        // Main thread transformation (mirrors worker logic)
        _transformReportsMainThread(reports) {
            const now = new Date();
            return reports.map(raw => this._transformReportMainThread(raw, now));
        },

        _transformReportMainThread(raw, now) {
            const dateValue = raw.date || raw.createdOn;
            const reportDate = dateValue ? new Date(dateValue) : null;
            const daysDiff = reportDate && !isNaN(reportDate.getTime())
                ? DateUtils._getDaysDifference(reportDate, now)
            ...
            // Compute visibility using ExtraVisibility logic
            const visibility = {
                perpetrator: !!(raw.perpetrators?.length > 0 || raw.perpetrator),
                perp: !!(raw.perpetrators?.length > 0 || raw.perpetrator),
                settlement: !!(raw.settlement?.name || raw.settlement?.slug),
                place: !!(raw.place?.name || raw.place?.slug || raw.locationType?.name || raw.locationType?.slug),
                locationType: !!(raw.place?.name || raw.place?.slug || raw.locationType?.name || raw.locationType?.slug),
                backer: !!(raw.backer?.name || raw.backer?.slug),
                description: !!(raw.description?.trim()),
                info: !!(raw.description?.trim()),
                videos: !!(raw.videos?.length > 0),
                images: !!(raw.reportImages?.length > 0),
                reportImages: !!(raw.reportImages?.length > 0),
                photo: !!(raw.photo?.url),
                victimsDonationLink: !!(raw.victimsDonationLink),
                victims: !!(raw.victimsDonationLink),
                reporterEventLink: !!(raw.reporterEventLink),
                urgent: raw.urgent === true,
                topic: !!(raw.topic?.length > 0 || raw.topic?.slug),
                reporters: !!(raw.reporters?.length > 0),
                reporter: !!(raw.reporters?.length > 0),
                locality: !!(raw.locality?.name || (Array.isArray(raw.locality) && raw.locality.length > 0)),
                region: !!(raw.subRegion?.name || raw.region?.name ||
                          (Array.isArray(raw.subRegion) && raw.subRegion.length > 0) ||
                          (Array.isArray(raw.region) && raw.region.length > 0)),
                territory: !!(raw.territory?.name || (Array.isArray(raw.territory) && raw.territory.length > 0)),
                multiLocality: !!raw.hasMultipleLocalities,
                multiRegion: !!raw.hasMultipleSubRegions,
                multiTerritory: !!raw.hasMultipleTerritories,
                multiGovernorate: !!raw.hasMultipleRegions
            };
            ...
            return {
                ...
                slug: raw.slug,
                name: raw.name,
                formattedDate: DateUtils.format(dateValue),
                formattedDateForTag: DateUtils.formatForTag(dateValue),
                reporterNamesFormatted: formatReporterNames(raw.reporters),
                daysDiff: daysDiff,
                reportUrl: reportUrl,
    ...
    function populateReporterBylineLinks(itemElement, reporters) {
        ...
        validReporters.forEach((reporter, index) => {
            ...
            // Use pre-computed URL if available (from worker)
            reporterLink.href = reporter.url ?? `/reporter/${reporter.slug}`;
    ...
    // Uses pre-computed values from DataWorker when available
    function populateBasicFields(itemElement, reportData) {
        ...
        // Use pre-computed photoUrl if available (from worker), fallback to raw data
        const photoUrl = reportData.photoUrl ?? reportData.photo?.url ?? '';
        if (DOM.setImage(DOM.$(SELECTORS.mainImage, itemElement), photoUrl, reportData.name)) successCount++;

        // Use pre-computed formattedDate if available (from worker)
        const dateDisplay = reportData.formattedDate ?? DateUtils.format(reportData.date || reportData.createdOn);
        if (DOM.setText(DOM.$('[cms-field="date"]', itemElement), dateDisplay)) successCount++;

        // Use pre-computed reporterNamesFormatted if available (from worker)
        const reporterNames = reportData.reporterNamesFormatted ?? formatReporterNames(reportData.reporters);
        DOM.setText(DOM.$('[cms-field="reporters"]', itemElement), reporterNames);

        // Handle topics - use pre-computed topics array if available (from worker)
        const topicsWrap = DOM.$('[cms-wrap="topics"]', itemElement);
        const topicLinkTemplate = DOM.$('[cms-link="topic"]', itemElement);
        const topics = reportData.topics ?? (Array.isArray(reportData.topic) ? reportData.topic : (reportData.topic?.slug ? [reportData.topic] : []));
        ...
        if (topicsWrap && topicLinkTemplate) {
            ...
            if (topics.length > 0) {
                ...
                topics.forEach((topic, index) => {
                    ...
                    // Use pre-computed URL if available (from worker)
                    topicLink.href = topic.url ?? `/topic/${topic.slug}`;
        ...
        // Handle urgent wrapper - use pre-computed showUrgentBadge if available (from worker)
        const urgentWrap = DOM.$('[cms-wrap="urgent"]', itemElement);
        if (urgentWrap) {
            let showUrgent = false;
            if (reportData.showUrgentBadge !== undefined) {
                // Use pre-computed value from worker
                showUrgent = reportData.showUrgentBadge;
            } else if (reportData.urgent === true) {
        ...
        // Handle report links - use pre-computed reportUrl if available (from worker)
        DOM.$$('[cms-link="report-link"]', itemElement).forEach(link => {
            const reportUrl = reportData.reportUrl ?? reportData.reporterEventLink ??
    ...
    function populateReporterItem(reporterItem, reporter) {
        ...
        if (reporterLink) {
            // Use pre-computed URL if available (from worker)
            DOM.setLink(reporterLink, reporter.url ?? (reporter.slug ? `/reporter/${reporter.slug}` : null));

            const nameElement = DOM.$('[cms-field="reporter"]', reporterLink);
            if (nameElement) DOM.setText(nameElement, reporter.name);

            // Use pre-computed photoUrl if available (from worker)
            const photoUrl = reporter.photoUrl ?? reporter.photo?.url ?? reporter.photo;
        ...
        // Use pre-computed photoUrl if available (from worker)
        const imgPhotoUrl = reporter.photoUrl ?? reporter.photo?.url ?? reporter.photo;
        const imgEl = DOM.$('[reporter-image="true"]', reporterItem);
        if (imgEl && imgPhotoUrl) {
            imgEl.src = imgPhotoUrl;
    ...
    function populateReporterInfo(itemElement, reporters) {
        ...
        // Use pre-computed URL if available (from worker)
        DOM.setLink(reporterLinkElement, firstReporter.url ?? `/reporter/${firstReporter.slug}`);

        if (reporters.length === 1) {
            DOM.setText(singleReporterNameElement, firstReporter.name);
            // Use pre-computed photoUrl if available (from worker)
            const photoUrl = firstReporter.photoUrl ?? firstReporter.photo?.url ?? firstReporter.photo;
            if (singleReporterImageElement && photoUrl) {
                singleReporterImageElement.src = photoUrl;
                singleReporterImageElement.alt = firstReporter.name;
            }
            // Use pre-computed URL if available (from worker)
            DOM.setLinks('a[cms-link="reporter"]', itemElement, firstReporter.url ?? `/reporter/${firstReporter.slug}`);
            ...
            // Use pre-computed photoUrl if available (from worker)
            const firstPhotoUrl = reporters[0].photoUrl ?? reporters[0].photo?.url ?? reporters[0].photo;
            if (firstReporterImage && firstPhotoUrl) {
                firstReporterImage.src = firstPhotoUrl;
                firstReporterImage.alt = reporters[0].name;
            }
            const secondPhotoUrl = reporters[1]?.photoUrl ?? reporters[1]?.photo?.url ?? reporters[1]?.photo;
            if (secondReporterImage && secondPhotoUrl) {
                secondReporterImage.src = secondPhotoUrl;
    ...
    function populatePerpetratorInfo(itemElement, reportData) {
        ...
        // Use pre-computed perpetrator if available (from worker)
        let perpetrator = reportData.perpetrator;
        if (!perpetrator && reportData.perpetrators?.length > 0) {
        ...
        if (perpetrator) {
            // Use pre-computed URL if available (from worker)
            if (perpLink && (perpetrator.url || perpetrator.slug)) {
                perpLink.href = perpetrator.url ?? `/perp/${perpetrator.slug}`;
        ...
        // Settlement - use pre-computed URL if available (from worker)
        ...
        if (reportData.settlement) {
            if (settlementLink && (reportData.settlement.url || reportData.settlement.slug)) {
                settlementLink.href = reportData.settlement.url ?? `/settlement/${reportData.settlement.slug}`;
        ...
        // Place/location type - use pre-computed URL if available (from worker)
        const placeLink = DOM.$('a[cms-link="place"]', perpInfoWrap);
        const placeField = DOM.$('div[cms-field="Place"]', perpInfoWrap);
        const placeData = reportData.place || reportData.locationType;
        if (placeData) {
            const placeName = placeData.name || placeData;
            if (placeLink && (placeData.url || placeData.slug)) {
                placeLink.href = placeData.url ?? `/place/${placeData.slug}`;
        ...
        // Backer - use pre-computed URL if available (from worker)
        ...
        if (reportData.backer) {
            if (backerLink && (reportData.backer.url || reportData.backer.slug)) {
                backerLink.href = reportData.backer.url ?? `/backer/${reportData.backer.slug}`;
    ...
    function populateVideos(itemElement, videos) {
        ...
        videos.forEach((video, index) => {
            ...
            if (iframe && video.url) {
                // Use pre-computed embedUrl from worker, fallback to inline conversion
    ...
    async function applyFilters(skipUrlUpdate = false) {
        ...
        try {
            ...
            // Use DataWorker for fetch + transform (off main thread)
            const workerResponse = await DataWorker.fetchReports(url, false);
            const items = workerResponse?.data || [];

            // Cache transformed reports for view switching
            Store.setState({ cachedReports: items }, true);

            const totalFromMetadata = workerResponse?.metadata?.total || items.length;
            Store.setState({
                currentOffset: CONFIG.REPORTS_LIMIT,
                totalReports: totalFromMetadata,
                hasMoreReports: CONFIG.REPORTS_LIMIT < totalFromMetadata
            }, true);
    ...
    async function loadMoreReports() {
        ...
        pendingLoadRequest = (async () => {
            try {
                ...
                // Use DataWorker for fetch + transform (off main thread)
                const workerResponse = await DataWorker.fetchReports(url, true);
                ...
                // Handle cancelled request (returns null)
                if (!workerResponse) {
                    log('Request #' + requestId + ' was cancelled');
                    return;
                }
                ...
                const totalReports = workerResponse.metadata?.total || state.totalReports;
    ...
    async function loadReports(initializeUI = true) {
        setCmsLoadingIndicator(true);
        try {
            // Initialize DataWorker early (non-blocking)
            // This starts worker initialization in parallel with DOM setup
            const workerInitPromise = DataWorker.init();
            ...
            // Wait for worker to be ready before making fetch requests
            ...
            // Use DataWorker for fetch + transform (off main thread)
            const workerResponse = await DataWorker.fetchReports(initialUrl, false);
            const items = workerResponse?.data || [];
            ...
            const totalReports = workerResponse?.metadata?.total || items.length;
            Store.setState({
